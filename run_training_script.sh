...
process_address() {
    ...
    torch_cmd="torchrun --standalone --nproc_per_node=1 -m future_attention.training_script --config_file future_attention/train_configs/small.yaml --train datasets/wikipedia/ --platform_type LAMBDA --aws_access_key_id ${aws_access_key} --aws_secret_access_key ${aws_secret_key} --sweep_id qoevh5um --sweep_count 6 --sync_profile_live True"
