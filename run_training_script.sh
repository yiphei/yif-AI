...
process_address() {
    ...
    torch_cmd="torchrun --standalone --nproc_per_node=1 -m future_attention_transformer.training_script --config_file future_attention_transformer/train_configs/small.py --train datasets/wikipedia/ --platform_type LAMBDA --aws_access_key_id ${aws_access_key} --aws_secret_access_key ${aws_secret_key} --sweep_id a5e8ggsj --sweep_count 1 --sync_profile_live True"
