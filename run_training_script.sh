...
process_address() {
    ...
    tmux send-keys -t mySession "torchrun --standalone --nproc_per_node=1 -m parallel_future_transformer.training_script --config_file parallel_future_transformer/train_configs/small.py --train datasets/wikipedia/ --platform_type LAMBDA --aws_access_key_id ${aws_access_key} --aws_secret_access_key ${aws_secret_key} --sweep_id a5e8ggsj --sweep_count 1 --sync_profile_live True" C-m
    exit
EOF
