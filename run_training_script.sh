...
process_address() {
    ...
    tmux send-keys -t mySession "torchrun --standalone --nproc_per_node=1 -m transformer_dropout.training_script --config_file transformer_dropout/train_configs/attn_dropout_profile.py --train datasets/full_harry_potter_char/ --platform_type LAMBDA --aws_access_key_id ${aws_access_key} --aws_secret_access_key ${aws_secret_key} --sync_profile_live True" C-m
    exit
EOF
