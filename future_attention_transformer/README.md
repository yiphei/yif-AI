...
Note that $A_{unmasked}$ and $A_{future}$ have different shapes, so merging the two requires padding operations, which is denoted by $\cup$. Also note that $out_{unmasked}$ is different than $out_{causal}$ because the former's softmax is on the union of $A_{unmasked}$ and $A_{future}$.
