...
process_address() {
    ...
    ssh -i ~/.ssh/id_jarvis_rsa -o StrictHostKeyChecking=no -p 11214 "$address" <<EOF
        cd $HOME
        apt-get update -y
...
EOF
    ...
    git ls-files | rsync -avz --files-from=- -e "ssh -i ~/.ssh/id_jarvis_rsa -o StrictHostKeyChecking=no -p 11214" ./ "$address":~/yif-AI/

    # SSH into the server, start a tmux session, and run the commands
    ssh -i ~/.ssh/id_jarvis_rsa -o StrictHostKeyChecking=no -p 11214 "$address" <<EOF
    tmux new-session -d -s mySession /bin/bash
    tmux send-keys -t mySession "cd $HOME" C-m
    ...
    tmux send-keys -t mySession "torchrun --standalone --nproc_per_node=1 -m transformer_dropout.training_script --config_file transformer_dropout/train_configs/baseline_profile.py --train datasets/openweb/ --platform_type LAMBDA --aws_access_key_id ${aws_access_key} --aws_secret_access_key ${aws_secret_key} --sweep_id 04m3u00a --sweep_count 100000 --save_checkpoint True --save_model True --sync_profile_live True" C-m
    exit
EOF
